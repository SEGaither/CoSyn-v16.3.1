...
function Find-AnchorMatches {
    ...
    foreach ($anchor in $AnchorDefinitions) {
        ...
        foreach ($file in $files) {
            ...
            for ($i = 0; $i -lt $lines.Length; $i++) {
                if ($lines[$i] -match $anchor.pattern) {
                    $anchorMatches += [pscustomobject][ordered]@{
                        file = $file.Name
                        line = $i + 1
                        text = $lines[$i].Trim()
                    }
        ...
        if ($anchorMatches.Count -lt $anchor.minimum_matches) {
            Stop-Preflight -Code 'CHANGE_MAP_ANCHOR_MISSING' -Message (
                "Defect $($anchor.defect_id) anchor '$($anchor.label)' expected at least " +
                "$($anchor.minimum_matches) match(es), found $($anchorMatches.Count). Baseline differs from the expected reviewed state."
            )
        }
