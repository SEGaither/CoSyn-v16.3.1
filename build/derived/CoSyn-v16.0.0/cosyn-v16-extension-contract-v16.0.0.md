...
**Artifact:** `cosyn-v16-extension-contract-v16.0.0.md`
**Version:** 16.0.0
**Tier:** Core (Tier 1)
...
**Status:** PROPOSED — pending Creator ratification
...
This document defines the contract every conforming package MUST satisfy. Instances of the contract are provided in `cosyn-v16-bind-template-v16.0.0.json` and in each bolt-on's own bind template.

## 1. Governing Rules

1. A lower layer MAY extend a higher layer but MUST NOT restate, weaken, fork, or compete with higher governance.
2. Each governing capability has exactly one authoritative owner (see `capability-ownership-map.json`).
...
| `authority_level` | `constitutional_root`, `subordinate_specification`, `subordinate_enforcement`, `subordinate_routing`, `subordinate_registry`, `subordinate_discipline`, `task_specific_bolt_on`, `task_specific_bolt_on_extension`, `informative_reference`, or `project_specific` |
...
| `integrity_hashes` | SHA-256 hash of every manifest_files entry |
...
2. **Load Tier-2 User Profile if present.** Verify manifest. Verify that Tier-2 supplies only capabilities in the Tier-2 reserved space (user-response, user-voice-style, user-editing-preference). Verify that Tier-2's `required_core_generation` matches loaded Core. Reject Tier-2 packages that supply Core-owned capabilities.
...
*End of `cosyn-v16-extension-contract-v16.0.0.md` — PROPOSED / READY FOR CREATOR REVIEW*
