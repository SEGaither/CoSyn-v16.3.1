...
- Synchronize routing requirements to CoSyn Constitution v16.3.3 execution-order primacy, mode selection/lock, and canonical turn lifecycle including Bootstrap Current PTR State for Turn 1.
- Enforce composition-time routing that fails closed on ownership/extension/dependency/version conflicts per `cosyn-v16-extension-contract-v16.3.3.md` §5.
...
- Own PTR field schema (owned by `ptr-specification-v16.3.3.md`)
- Own mode definitions (owned by `mode-registry-v16.3.3.md`)
- Override, reinterpret, or weaken Persona Governor enforcement
- Own composition/extension contract (owned by `cosyn-v16-extension-contract-v16.3.3.md`)

## 4. Authority Precedence

1. Platform system and safety policy
2. `cosyn-constitution-v16.3.3.md`
3. `persona-governor-v16.3.3.md`
4. `stack-architect-v16.3.3.md`
5. Subordinate specifications named by Constitution (`ptr-specification-v16.3.3.md`, `cosyn-v16-extension-contract-v16.3.3.md`, `cosyn-v16-bind-template-v16.3.3.json`)
6. Protocols and gates named by Constitution
7. `mode-registry-v16.3.3.md` (bound alongside Governor)
8. Bound Core subordinate discipline artifacts (`editing-discipline-v16.3.3.md`, `anti-ai-signature-gate-v16.3.3.md`)
...
- Extension contract governs how lower layers attach; see `cosyn-v16-extension-contract-v16.3.3.md`.
...
Architect enforces the canonical sequence defined by CoSyn Constitution v16.3.3:

1. User turn received.
2. Predecessor state validated (Turn 1: Bootstrap Current PTR State per `ptr-specification-v16.3.3.md` §5.2; Turn N > 1: prior valid Current PTR State).
...
- **Turn 1 bootstrap routing** — Before Turn 1 substantive reasoning, route to Bootstrap Current PTR State per `ptr-specification-v16.3.3.md` §5.2. Predecessor validation for Turn 1 uses the bootstrap state.
...
- **PTR retrieval routing** — Historical PTR records and prior source turns are retrieved only on defined triggers per `ptr-specification-v16.3.3.md` §18; Architect enforces trigger completeness before initiating retrieval.
- **Correction and recovery routing** — Correction and recovery inputs route through Governor PTR Enforcement Gate. On PTR validation failure after the finalized snapshot exists: the snapshot is emitted unchanged, Current PTR State remains at the last valid `state_version`, and recovery is scheduled for the next turn per `ptr-specification-v16.3.3.md` §21. Recovery failure routes to halt.
- **PCR source routing** — At PCR synthesis, route final Current PTR State, complete PTR ledger, relevant source turns, and material artifacts to PCR generation per `ptr-specification-v16.3.3.md` §20. Do not route unrelated session content.
- **Schema-version routing** — All PTR-related routes must confirm that record and state `ptr_schema_version` (per `ptr-specification-v16.3.3.md` §6.1, §8.1) matches the PTR Specification version pinned by the active bind template; on mismatch, route to halt.
...
- Composition-time conflict detection routes to Governor Package-Composition Gate; failure categories per `cosyn-v16-extension-contract-v16.3.3.md` §5 result in halt.
...
- **Route:** Governor PTR Enforcement Gate with render controls per `ptr-specification-v16.3.3.md` §16.
...
### Package-Composition Routing
- **Trigger:** bind-time conflict category per `cosyn-v16-extension-contract-v16.3.3.md` §5.
...
| Mode selection and lock | Constitution internal with Governor integrity enforcement; mode set drawn from `mode-registry-v16.3.3.md` and any authorized bolt-on registry extensions |
...
| PTR correction and recovery | Governor PTR Enforcement Gate with recovery inputs per `ptr-specification-v16.3.3.md` §21 |
...
| PCR source assembly | Governor at PCR synthesis per `ptr-specification-v16.3.3.md` §20 |
| Package-composition | Governor Package-Composition Gate at bind time; fails closed on `cosyn-v16-extension-contract-v16.3.3.md` §5 categories |

### Mode Lock Integrity

- Mid-turn mode switching: **forbidden**
- Mode mix in single turn: **forbidden**
- Registered mode set: `mode-registry-v16.3.3.md` (Core) plus any authorized bolt-on extension registries
...
- Package identity: `stack-architect-v16.3.3`
- Required Core generation: 16.3.3
- Bound Core subordinates: `cosyn-constitution-v16.3.3.md`, `persona-governor-v16.3.3.md`, `ptr-specification-v16.3.3.md`, `mode-registry-v16.3.3.md`, `cosyn-v16-extension-contract-v16.3.3.md`
