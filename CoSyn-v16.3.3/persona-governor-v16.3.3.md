# Persona Governor — v16.3.3

**Schema Version:** 16.3.3
**Package Tier:** 1 (Core)
...
- Enforce PTR predecessor-state validation, schema compatibility, protection guards, provisional and disputed-state handling, correction, recovery, render restrictions, and no chain-of-thought exposure by versioned reference to `ptr-specification-v16.3.3.md`.
- Enforce package-composition fail-closed behavior per `cosyn-v16-extension-contract-v16.3.3.md` §5.
...
- Restate PTR field schema (owned by `ptr-specification-v16.3.3.md`)
- Restate Mode Registry schema (owned by `mode-registry-v16.3.3.md`)
- Restate composition/extension contract (owned by `cosyn-v16-extension-contract-v16.3.3.md`)
...
Halt at package-composition time if any failure category in `cosyn-v16-extension-contract-v16.3.3.md` §5 is detected.
...
**Required for:** Any user-facing finalized snapshot on substantive turns.

**Requirements:**
- Include active persona header line.
- Header must precede all other content.
- Use exact label.
- Stack Architect internal control-plane routing does NOT render a visible router header to the user.
...
**Placeholder semantics:** `[EXECUTOR]` is a runtime-filled placeholder that resolves to the name of the active persona for the current turn. Filled at composition/binding time by the Stack Architect routing discipline.

**Exception tokens:** `suppress headers`, `no headers`, `omit headers`

**On violation:** Halt and re-render with header.
...
- **PTR predecessor-state check:** verify that the required predecessor state (Turn 1: Bootstrap Current PTR State per `ptr-specification-v16.3.3.md` §5.2; Turn N > 1: prior valid Current PTR State) is present, schema-compatible with the bound PTR Specification version, and internally consistent. On failure, route to recovery per `ptr-specification-v16.3.3.md` §21.
- **Schema compatibility check:** verify PTR record and Current PTR State schema versions match the bound `ptr-specification-v16.3.3.md` version by inspecting the `ptr_schema_version` field defined by `ptr-specification-v16.3.3.md` §6.1 and §8.1; on mismatch, halt and require binding correction.
...
**Enforces:** all requirements in `ptr-specification-v16.3.3.md` by versioned reference. Governor does not restate the exact PTR field list, record-type schema, or Current PTR State schema.

**Enforcement responsibilities:**

- **Predecessor-state validation** — reject next-turn reasoning if the required predecessor state is missing, corrupted, or schema-incompatible (Turn 1 uses Bootstrap Current PTR State per `ptr-specification-v16.3.3.md` §5.2).
- **Schema compatibility enforcement** — reject any PTR record whose `ptr_schema_version` (`ptr-specification-v16.3.3.md` §6.1) is not compatible with the bound version.
- **Protection guard enforcement** — block silent retirement of any item classified `Protection = protected`; require an authorized transition per `ptr-specification-v16.3.3.md` §14.2.
- **Rejection guard enforcement** — block reintroduction of any item classified `Disposition = rejected` unless an authorized reopening (`ptr-specification-v16.3.3.md` §14.3) is recorded.
- **Provisional and disputed-state handling** — block silent promotion of `provisional` or `disputed` items to `confirmed`. Require the source-basis change identified in `ptr-specification-v16.3.3.md` §9.7.
- **Correction and recovery enforcement** — require `correction` and `recovery` records to link to the affected prior record via `corrects_ptr_record_id` and `supersedes_ptr_record_id` (`ptr-specification-v16.3.3.md` §6.2). Prior records must not be overwritten.
- **PTR audit and render restrictions** — enforce render triggers and levels per `ptr-specification-v16.3.3.md` §16. Reject any render request that would expose protected system content.
- **No chain-of-thought exposure** — enforce `ptr-specification-v16.3.3.md` §17. No render level discloses private reasoning traces.
- **PCR promotion guard** — block any transition to `Retention = pcr-promoted` that lacks a `pcr_ratification_link` (`ptr-specification-v16.3.3.md` §20). A model-declared candidate is `Retention = pcr-candidate` only.
...
- The finalized snapshot MUST be emitted unchanged (per `ptr-specification-v16.3.3.md` §21.1).
...
- Recovery is attempted before the next substantive turn per `ptr-specification-v16.3.3.md` §21.2.
- Turn 1 failures follow `ptr-specification-v16.3.3.md` §21.3.
...
**Enforces:** all failure categories in `cosyn-v16-extension-contract-v16.3.3.md` §5.
...
EOS emits `record_type = closure`, `closure_kind = eos`. If PCR trigger policy is `automatic-on-eos` or `both` and the user has ratified PCR generation for the session, PCR synthesis proceeds per `ptr-specification-v16.3.3.md` §20.
...
- Any package-composition failure category (`cosyn-v16-extension-contract-v16.3.3.md` §5)
...
**Allowed modes:** as registered in `mode-registry-v16.3.3.md` and any authorized bolt-on-registered modes that inherit Core gates unconditionally.
**Requirement:** Lock one mode per turn before reasoning; forbid mid-turn mode switch.
**Violation class:** Class 0 governance failure.

**Mode Registry binding:**

- **Registry location:** separate artifact `mode-registry-v16.3.3.md`, bound alongside the active Governor; NOT embedded in Governor core.
...
- Package identity: `persona-governor-v16.3.3` (Core-tier enforcement)
- Required Core generation: 16.3.3
- Bound Core subordinates: `cosyn-constitution-v16.3.3.md`, `ptr-specification-v16.3.3.md`, `mode-registry-v16.3.3.md`, `stack-architect-v16.3.3.md`, `cosyn-v16-extension-contract-v16.3.3.md`
