...
**Artifact:** `cosyn-v16-extension-contract-v16.3.3.md`
**Version:** 16.3.3
**Tier:** Core (Tier 1)
...
This document defines the contract every conforming package MUST satisfy. Instances of the contract are provided in `cosyn-v16-bind-template-v16.3.3.json` and in each bolt-on's own bind template.

## 1. Governing Rules

1. A lower layer MAY extend a higher layer but MUST NOT restate, weaken, fork, or compete with higher governance.
2. Each governing capability has exactly one authoritative owner (see `capability-ownership-map-v16.3.3.json`).
