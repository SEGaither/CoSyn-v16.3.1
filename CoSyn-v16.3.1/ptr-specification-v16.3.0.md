# Parsed Turn Record (PTR) Specification — v16.3.0

**Schema Version:** 16.3.0
**Package Tier:** 1 (Core)
**Role:** Subordinate specification bound by CoSyn Constitution v16.3.0
...
The PTR Specification is a subordinate specification bound by CoSyn Constitution v16.3.0. It is not a constitutional authority.

- CoSyn Constitution v16.3.0 owns the seven constitutional PTR principles CP1–CP7.
- `persona-governor-v16.3.0.md` enforces this specification.
- `stack-architect-v16.3.0.md` routes PTR creation, retrieval, correction, and recovery.
- `cosyn-v16-bind-template-v16.3.0.json` pins the active version.
...
- **Finalized response snapshot** — the immutable response snapshot produced by the Presentation Gate (CoSyn Constitution v16.3.0 Canonical Turn Lifecycle step 4–5). Also referred to as `finalized`, `frozen`, or `emission-locked`.
...
This specification inherits the canonical turn lifecycle from CoSyn Constitution v16.3.0. Summarized here for reference; the Constitution is authoritative:
...
Each completed user-assistant turn produces exactly one PTR ledger entry of one of the following types. Record types are enumerated; new record types require a subsequent PTR Specification version. Where PTR validation fails after a turn (§21.1), no committed valid entry is created for that turn; the subsequent recovery record (§21.2) accounts for the failed turn.
...
- Every completed user-assistant turn creates exactly one PTR ledger entry beginning with Turn 1. Where PTR validation fails (§21.1), no committed valid entry exists for that turn; the subsequent recovery record (§21.2) accounts for the failed turn.
...
|---|---|
| `ptr_schema_version` | current PTR Specification schema version (e.g., `"16.3.0"`) |
| `state_version` | `0` |
...
| `note` | string | optional | Short human-readable annotation for the record. Particularly applicable to `no_material_change` records (§7.2). |
...
- optional `note` field (defined in §6.1) permitted.
...
Silent retirement is prohibited (Class 0 failure per CoSyn Constitution v16.3.0).
...
- Task-specific bolt-ons MAY extend PCR schemas with domain-specific fields. Extensions MUST NOT restate promotion ratification rules, MUST NOT permit `pcr-promoted` without `pcr_ratification_link`, and MUST declare dependency on this §20 as their base ownership.
...
- attempted silent modification of the finalized response snapshot by PTR or telemetry processing (Class 0 per CoSyn Constitution v16.3.0).
...
- PTR Specification schema version is declared at the top of this file (`16.3.0`).
...
- Compatibility statement: PTR Specification v16.3.0 is compatible with CoSyn Constitution v16.3.0, Persona Governor v16.3.0, Stack Architect v16.3.0, Core Bind Template v16.3.0, Mode Registry v16.3.0, Extension Contract v16.3.0.
...
*Document ID: ptr-specification-v16.3.0 — PROPOSED / UNDER REFINEMENT — Generated 2026-08-14*
