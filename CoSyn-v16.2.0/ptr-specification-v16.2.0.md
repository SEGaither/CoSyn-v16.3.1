# Parsed Turn Record (PTR) Specification — v16.2.0

**Schema Version:** 16.2.0
**Package Tier:** 1 (Core)
**Role:** Subordinate specification bound by CoSyn Constitution v16.2.0
**Authority Level:** subordinate_specification
**Authority Ceiling:** `cosyn-constitution-v16.2.0.md`
**Status:** PROPOSED / UNDER REFINEMENT
**Supersedes source basis:** PTR Specification v1.2.2 (migration/reference source only)
**Refined from:** ptr-specification-v16.1.0.md (SHA-256 12EEAC1B4B58E40E53DF4414A82930CB37765DB9E7A1095C2E5072191C0115D2) — corrective refinement per Creator-authorized CCT pass
...
**v16.1.0 → v16.2.0 Corrective Refinement (D01, D02, D03, D07):**

- D01: Added `note` to §6.2 conditional fields, resolving the contradiction between §6.3 (which prohibits undefined fields) and §7.2 (which permitted an undefined optional `note` field). The field is now defined with exact type, applicability condition, and scope.
- D02: Replaced the unqualified "exactly one PTR ledger entry per completed turn" rule in §4 and §5.1 with the **committed-record invariant** — one committed valid record per completed turn, subject to failure semantics in §21. Added §21.10 defining deterministic turn-record boundary behavior under failure, covering `turn_id`, `ledger_sequence`, `prior_state_pointer`, `state_version`, and recovery-failure semantics.
- D03: Updated current normative example §25.1 to use `ptr_schema_version: "16.2.0"`. Updated §5.2 bootstrap table example value. Updated §23 compatibility statement. Genuinely historical examples remain at their historical values.
- D07: Replaced exact task-specific bolt-on filename reference in §20 with a capability-family reference.

---

## 1. Authority and Subordination

The PTR Specification is a subordinate specification bound by CoSyn Constitution v16.2.0. It is not a constitutional authority.

- CoSyn Constitution v16.2.0 owns the seven constitutional PTR principles CP1–CP7.
- `persona-governor-v16.2.0.md` enforces this specification.
- `stack-architect-v16.2.0.md` routes PTR creation, retrieval, correction, and recovery.
- `cosyn-v16-bind-template-v16.2.0.json` pins the active version.
...
- **Finalized response snapshot** — the immutable response snapshot produced by the Presentation Gate (CoSyn Constitution v16.2.0 Canonical Turn Lifecycle step 4–5). Also referred to as `finalized`, `frozen`, or `emission-locked`.
- **User-visible emission** — Canonical Turn Lifecycle step 9 only. Distinct from the finalized snapshot.
- **Committed record** — a PTR ledger record that has passed validation and been accepted as valid into the ledger. A record that fails validation is not committed.

## 3. Canonical Turn Lifecycle (Reference)

This specification inherits the canonical turn lifecycle from CoSyn Constitution v16.2.0. Summarized here for reference; the Constitution is authoritative:
...
Each completed user-assistant turn results in exactly one committed valid PTR ledger entry of one of the following types. This is the **committed-record invariant** (see §5.1). When PTR creation or validation fails for a completed turn, the failure semantics in §21 govern the outcome. Record types are enumerated; new record types require a subsequent PTR Specification version.
...
## 5. Turn 1 Bootstrap and Committed-Record Invariant

### 5.1 Committed-record invariant

Every completed user-assistant turn results in exactly one committed valid PTR ledger entry, beginning with Turn 1.

This is the **committed-record invariant**: it governs committed valid records, not record attempts. When PTR creation or validation fails for a turn, no record is committed for that turn; §21 defines the deterministic behavior including recovery and turn-record boundary semantics.
...
|---|---|
| `ptr_schema_version` | current PTR Specification schema version (e.g., `"16.2.0"`) |
| `state_version` | `0` |
...
- `ledger_sequence` advances by 1 for every committed turn record.
...
| `ledger_sequence` | integer ≥ 1 | required | Ordered position in the PTR ledger. Advances by 1 per committed turn record. |
...
| `note` | string | optional when `record_type = no_material_change` | Short informative note. Maximum one sentence. No governance authority; informational only. |
...
- optional short `note` field permitted (defined in §6.2).
...
Silent retirement is prohibited (Class 0 failure per CoSyn Constitution v16.2.0).
...
- Task-specific bolt-ons MAY extend PCR schemas with domain-specific fields by declaring `capabilities_extended` targeting `ptr.pcr-promotion-framework`. Extensions MUST NOT restate promotion ratification rules, MUST NOT permit `pcr-promoted` without `pcr_ratification_link`, and MUST declare dependency on this §20 as their base ownership.
...
### 21.10 Turn-record boundary behavior under failure

This section establishes deterministic semantics for the interaction between the committed-record invariant (§5.1) and failure semantics. It takes precedence over any interpretation that would make these two requirements logically incompatible.

**Whether the one-record invariant applies to record attempts or committed valid records:**
The committed-record invariant (§5.1) governs committed valid records. It does not require that every record attempt succeed. A failed record attempt does not constitute a committed record.

**Whether a failed completed turn temporarily or permanently lacks a ledger entry:**
A failed completed turn temporarily lacks a committed ledger entry. Its ledger entry is deferred to the recovery process (§21.2). A recovery record created in the next turn satisfies the committed-record invariant for the recovery turn; the failed turn itself remains without a committed record in the ledger.

**Whether later recovery creates a record representing the affected prior turn:**
A `recovery` record represents the recovery action taken in the recovery turn. It links to the affected prior turn via `recovery_inputs.source_turn_ref`. It does not retroactively fill the failed turn's ledger position; the failed turn remains absent from committed records and is documented by reference in the recovery record.

**How `turn_id` behaves across the failure/recovery boundary:**
The failed turn's `turn_id` is preserved in `recovery_inputs.source_turn_ref`. The recovery record carries its own `turn_id` (the recovery turn's identifier), not the failed turn's identifier.

**How `ledger_sequence` behaves:**
The failed turn's `ledger_sequence` was never committed. The recovery turn's record receives the next sequential `ledger_sequence` value. No ledger sequence number is reserved, held, or retroactively assigned for the failed turn.

**How `prior_state_pointer` behaves:**
The recovery record's `prior_state_pointer` points to the last committed valid PTR record (the predecessor of the failed turn), not to any uncommitted record.

**How `state_version` behaves:**
If recovery succeeds and materially changes Current PTR State, `state_version_after > state_version_before` in the recovery record, advancing per §5.4. If recovery produces no material state change, `state_version_after = state_version_before`.

**What happens if recovery itself fails:**
If recovery fails (`recovery_result = failed`), a `halt` record is issued for the recovery turn. The halt record satisfies the committed-record invariant for the recovery turn. Explicit user direction is required before any further substantive turn proceeds.
...
- attempted silent modification of the finalized response snapshot by PTR or telemetry processing (Class 0 per CoSyn Constitution v16.2.0).
...
- PTR Specification schema version is declared at the top of this file (`16.2.0`).
...
- Compatibility statement: PTR Specification v16.2.0 is compatible with CoSyn Constitution v16.2.0, Persona Governor v16.2.0, Stack Architect v16.2.0, Core Bind Template v16.2.0, Mode Registry v16.2.0, Extension Contract v16.2.0.
...
These examples are schema-consistent and illustrative. They are not additional authority. Current examples use schema version 16.2.0. Genuinely historical examples, if any, would be explicitly labeled historical/non-normative; none appear in this section.

### 25.1 Turn 1 bootstrap with material state change

```json
{
  "ptr_schema_version": "16.2.0",
  ...
  "record_created_at": "2026-08-14T00:00:00Z"
}
```
...
- No PTR record is committed as valid for this turn (committed-record invariant: failure semantics apply per §21.10).
- Next turn attempts recovery per §21.2.

### 25.7 Recovery

`record_type = "recovery"`, `recovery_inputs` enumerated per §21.4 and §21.10 (including `source_turn_ref` for the affected prior turn), `recovery_result = "succeeded"`.
...
*Document ID: ptr-specification-v16.2.0 — PROPOSED / UNDER REFINEMENT — Generated 2026-08-14*
